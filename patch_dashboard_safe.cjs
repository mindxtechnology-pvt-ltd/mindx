...
// 7. Add Products Tab in UI (Header Buttons)
// Use EXACT substring replacement for the Team Members button.
const oldTeamButtonStr = `              <button
...
const newTeamAndProductsButtonStr = `              <button
                onClick={() => { setActiveTab('team'); setSelectedMember(null); }}
                className={\`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold transition-all \${
                  activeTab === 'team' ? 'bg-zinc-800 text-white shadow' : 'text-zinc-400 hover:text-white'
                }\`}
...
content = content.replace(oldTeamButtonStr, newTeamAndProductsButtonStr);
...
// 9. Add "Add Product" Sidebar button
const oldAddTeamButtonStr = `                {activeTab === 'team' && (
                  <button
...
const newAddTeamAndProductButtonStr = `                {activeTab === 'team' && (
                  <button
                    onClick={openNewTeamForm}
                    className="flex items-center gap-1 bg-white hover:bg-zinc-200 text-black text-xs font-bold px-3.5 py-2 rounded-xl transition-all cursor-pointer"
                  >
...
content = content.replace(oldAddTeamButtonStr, newAddTeamAndProductButtonStr);

// 10. Add Products Sidebar List
const oldTeamListEnd = `                    <ChevronRight size={14} className="self-center text-zinc-700" />
                  </div>
                ))}
              </div>
            )}
...
const productsSidebarList = `                    <ChevronRight size={14} className="self-center text-zinc-700" />
                  </div>
                ))}
...
content = content.replace(oldTeamListEnd, productsSidebarList);

// 11. Add Product Forms HTML
const oldMainEnd = `            )}
          </main>`;
...
content = content.replace(oldMainEnd, productsFormsHtml);

// APPLY DARK MODE FIXES AS WELL IN THE SAME SCRIPT
...
console.log('Patched Dashboard.jsx completely and safely.');
